defmodule SaladBowlTest do
  ...
  defp finished_turn(context) do
    ...
    salad_bowl = Charades.create_salad_bowl(
      ...
      current_turn: Turn.new(
        %{card: "0", time_remaining_in_sec: 0, completed_cards: [{:correct, "1"}, {:correct, "2"}, {:skipped, "3"}, {:incorrect, "4"}]})})
    |> elem(1)
  ...
  describe "end turn" do
    ...
    test "move incorrect cards + current card back to deck and keep correct out", %{finished_turn: salad_bowl} do
      slug = start_game(salad_bowl)
      {:ok, game} = SaladBowlAPI.end_turn(slug)
    ...
    test "finished round", %{finished_turn: salad_bowl} do
      finished_round_turn = Turn.new(
        %{card: nil, time_remaining_in_sec: 15, completed_cards: [{:correct, "1"}, {:correct, "2"}, ]})
      slug = %{salad_bowl | current_turn: finished_round_turn}
      |> start_game()
    ...
    test "finished game", %{finished_turn: salad_bowl} do
      finished_round_turn = Turn.new(
        %{card: nil, time_remaining_in_sec: 15, completed_cards: [{:correct, "1"}, {:correct, "2"}, ]})
      slug = %{salad_bowl | current_turn: finished_round_turn, current_round: "Charades"}
      |> start_game()
