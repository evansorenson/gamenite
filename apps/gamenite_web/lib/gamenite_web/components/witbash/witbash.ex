    ...
    {#if not @game.finished?}
      <div class="flex flex-col justify-center items-center space-y-4">
        ...
        <Timer time_remaining={@game.voting_timer.time_remaining} />
          <div class="flex flex-wrap justify-center font-serif px-4 pb-8">
            <h1 class="xs:text-4xl sm:text-5xl md:text-6xl text-gray-dark font-bold text-center">{@game.current_prompt.prompt}</h1>
          </div>
          {#if @game.current_prompt.scored?}
            <Answers prompt={@game.current_prompt} {=@roommates} {=@user_id} show_votes?={true}/>
          {#elseif @user_id not in @game.submitted_user_ids and (@user_id not in @game.current_prompt.assigned_user_ids or @game.final_round?)}
            ...
            <h2 class="text-4xl text-center pb-4">Vote for your favorite answer!</h2>
